...
from icdl.models.layers import BatchNormAct2d, create_norm_act, BlurPool2d, create_classifier
...
class DenseLayer(nn.Module):
    def __init__(self, num_input_features, growth_rate, bn_size, norm_layer=nn.BatchNorm2d, act_layer=nn.ReLU,
...
class DenseBlock(nn.ModuleDict):
    ...
    def __init__(self, num_layers, num_input_features, bn_size, growth_rate, norm_layer=nn.BatchNorm2d, act_layer=nn.ReLU,
                 drop_rate=0., memory_efficient=False):
...
class DenseTransition(nn.Sequential):
    def __init__(self, num_input_features, num_output_features, norm_layer=nn.BatchNorm2d, act_layer=nn.ReLU, aa_layer=None):
        super(DenseTransition, self).__init__()
        self.add_module('norm', norm_layer(num_input_features))
        self.add_module('act', act_layer(inplace=True))
...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16), bn_size=4, stem_type='',
                 ...
                 norm_layer=nn.BatchNorm2d, act_layer=nn.ReLU, aa_layer=None, drop_rate=0, memory_efficient=False,
                 aa_stem_only=True):
